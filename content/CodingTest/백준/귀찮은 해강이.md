...
두 번째 줄부터는 _M_줄에 걸쳐 _i_와 _j_(1 ≤ _i_, _j_ ≤ _N_)가 주어진다. 이는 _i_번 건물과 _j_번 건물이 연결되어 있다는 의미이다. 건물이 자기 자신과 연결되거나, 이미 연결된 건물의 쌍이 다시 주어지는 경우는 없다.

마지막 줄에는 _N_개의 강의코드 _Ai_(1 ≤ _Ai_ ≤ _N_)로 이루어진 강의 시간표가 공백으로 구분되어 주어진다.
